...
def grid_sample(image, optical):
    ...
    # out_val = torch.gather(image, 2, (iy_nw * IW + ix_nw).long().view(N, 1, H * W).repeat(1, C, 1)).view(N, C, H, W)
...
def index(feat, uv):
    ...
    # samples = torch.nn.functional.grid_sample(feat, uv, align_corners=False)  # [B, C, N, 1]
...
def perspective(points, calibrations, transforms=None):
    ...
    rot = calibrations[:, :3, :3]
    trans = calibrations[:, :3, 3:4]
    val = calibrations[:,3,0]*points[:,0,:] + calibrations[:,3,1]*points[:,1,:] + \
        calibrations[:,3,2]*points[:,2,:] + calibrations[:, 3, 3]
    ...
    # xyz = torch.cat([xy, val.unsqueeze(0)], 1)
