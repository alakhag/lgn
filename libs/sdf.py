...
def batch_eval(points, eval_func, num_samples=256 * 256 * 256):
    ...
    for i in range(num_batches):
        sdf[i * num_samples:i * num_samples + num_samples] = eval_func(points[:, i * num_samples:i * num_samples + num_samples])
...
def eval_grid_octree(coords, eval_func,
                     init_resolution=8, threshold=0.001,
                     num_samples=256 * 256 * 256):
    resolution = coords.shape[1:4]

    sdf = np.ones(resolution)
    ...
    while reso > 0:
        print ('\t', reso)
        # subdivide the grid
        grid_mask[0:resolution[0]-1:reso, 0:resolution[1]-1:reso, 0:resolution[2]-1:reso] = True
...
def get_winding(samples, obj_file):
    fs = open('garment_pts.obj', 'w')
    for [X,Y,Z] in tqdm(samples):
        fs.write('v '+str(X)+' '+str(Y)+' '+str(Z)+' 1 1 1\n')
    fs.close()

    os.system('~/libigl/build/tutorial/Calc_Winding_num_bin '+obj_file+' garment_pts.obj winding_num.txt;')
    label_occ = np.zeros(samples.shape[0])
    label_npt = np.zeros((samples.shape[0],3))
    label_nf = np.zeros((samples.shape[0]))

    winding_file = open('winding_num.txt').readlines()
    winding_num = np.array([float(x.strip()) for x in winding_file])
    for i in range(winding_num.shape[0]):
        winding_num[i] = winding_num[i]*winding_num[i] - winding_num[i] + 0.75*0.25
